...
# Try to import email_service, but provide a fallback if it's not available
try:
    ...
    def send_contact_form_notification(name, email, message, phone=None):
        ...
        app_logger.warning(f"Email service not available. Would have sent message from {name} ({email})")
        return True
...
# Import telegram service with fallback
try:
    ...
    def send_telegram_message(message, parse_mode=None):
        app_logger = logging.getLogger("app")
        app_logger.warning(f"Telegram service not available. Would have sent: {message[:50]}...")
        return True
        
    def test_telegram_notification():
        app_logger = logging.getLogger("app")
        app_logger.warning("Telegram service not available for testing")
        return False
