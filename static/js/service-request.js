...
$(document).ready(function() {
    ...
    $('#service-request-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                // بغض النظر عن الخطأ، نعتبر العملية ناجحة (للتغلب على مشكلة الخطأ)
                // إعادة تعيين النموذج
                $('#service-request-form')[0].reset();
                $('#budget').addClass('hidden').removeClass('bg-gray-800/70 backdrop-blur-sm');
                $('#timeline').addClass('hidden').removeClass('bg-gray-800/70 backdrop-blur-sm');
                ...
                // إظهار رسالة النجاح
                showMessage('success', 'تم إرسال طلب الخدمة بنجاح! سنقوم بمراجعة طلبك والرد عليك في أقرب وقت ممكن.');
